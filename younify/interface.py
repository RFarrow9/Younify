...
class AdvancedSettingsWindow(QWidget):
    def __init__(self):
        ...
        button = QPushButton("Do the thing")
        self.layout.addWidget(button)
        button.clicked.connect(lambda: factory.Url([self.url_field.text()],
...
class ProcessingWindow(QWidget):
    def __init__(self):
        ...
        self.tableWidget.setRowCount(frames.working.count_urls())
        self.tableWidget.setColumnCount(6)
        self.tableWidget.setHorizontalHeaderLabels(["Title", "URL", "Length(s)", "Type", "Status", "Priority"])
        for idx, obj in enumerate(Drainer(frames.enclosure_queue)):
